...
task :post do
  ...
  title = ENV["title"] || ask("What shall we call this post?", false)
  begin
  ...
  filename = File.join(CONFIG['posts'], "#{date}-#{slug(title)}.#{CONFIG['post_ext']}")
...
NOISE = %w(it its the and of)

def slug(title)
  parts = title.downcase.strip.squeeze(' ').gsub(/[^ a-z0-9-]/, '').split(' ')
  parts.reject{|w| NOISE.include? w }.join('-')
end
